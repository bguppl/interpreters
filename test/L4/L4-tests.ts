...
import { evalParse } from '../../src/L4/L4-eval';
...
describe('L4 Eval', () => {
    ...
    it('evaluates procedures', () => {
        expect(evalParse("(lambda (x) x)")).to.deep.equal(makeClosure([makeVarDecl("x")], [makeVarRef("x")], makeEmptyEnv()));
    });
    ...
    it('returns an error for recursive procedures', () => {
        expect(isError(evalParse("(L4 (define f (lambda (x) (if (= x 0) 1 (* x (f (- x 1)))))) (f 3))"))).to.be.true;
    });
