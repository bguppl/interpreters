...
describe('L1 Eval', () => {
    ...
    describe("Failures", () => {
        it("returns a Failure when accessing a variable in an empty env", () => {
            ...
            const result = bind(parseL1Exp(p("(+ y 2)")),
        ...
        it("returns a Failure for an unknown primitive op", () => {
            expect(bind(parseL1Exp(p("(eq? 1 1)")), exp => evalSequence([exp], makeEmptyEnv()))).to.satisfy(isFailure);
        })
    });
});
