    ...
    it('evaluates higher-order functions', () => {
        expect(bind(parseL5(`
            (L5 (define map
                ...
                (map (lambda (x) (* x x)) '(1 2)))`), evalProgram)).toEqual(makeOk(makeCompoundSExp(1, makeCompoundSExp(4, makeEmptySExp()))));
                  ...
                  (lambda (pred l)
                    (if (empty? l)
                        ...
                        (if (pred (car l))
                            ...
                            (filter pred (cdr l))))))
                (filter (lambda (x) (not (= x 2))) '(1 2 3 2)))`), evalProgram)).toEqual(makeOk(makeCompoundSExp(1, makeCompoundSExp(3, makeEmptySExp()))));
