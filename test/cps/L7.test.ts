...
describe('L7 Eval', () => {
    it('evaluates data type literals', () => {
        ...
        expect(evalParse("'(1)")).toEqual(makeOk(makeCompoundSExp(1, makeEmptySExp())));
    });
    ...
    it('evalautes "define" expressions', () => {
        expect(evalP("(L5 (define x 1) (+ x x))")).toEqual(makeOk(2));
        expect(evalP("(L5 (define x 1) (define y (+ x x)) (* y y))")).toEqual(makeOk(4));
    });
    ...
    it('applies procedures', () => {
        ...
        expect(evalP("(L5 (define square (lambda (x) (* x x))) (square 3))")).toEqual(makeOk(9));
        expect(evalP("(L5 (define f (lambda (x) (if (> x 0) x (- 0 x)))) (f -3))")).toEqual(makeOk(3));
    });

    it('evaluates recursive procedures', () => {
        expect(evalP("(L5 (define f (lambda (x) (if (= x 0) 1 (* x (f (- x 1)))))) (f 3))")).toEqual(makeOk(6));
    });
            ...
            (L5 (((lambda (f) (f f))
                  (lambda (fact)
                    (lambda (n)
                      (if (= n 0)
                          1
                          (* n ((fact fact) (- n 1))))))) 6))`)).toEqual(makeOk(720));
    });
    ...
    it('evaluates higher-order functions', () => {
        ...
        expect(evalP(`
            (L5 (define compose (lambda (f g) (lambda (x) (f (g x)))))
                ((compose not number?) 2))`)).toEqual(makeOk(false));
    });
    ...
    it('evaluates CPS functions iteratively', () => {
        expect(evalP(`
            (L5 (define sumCPS
                ...
                (sumCPS 1000 (lambda (x) x)))`)).toEqual(makeOk(500500));
    });
});
