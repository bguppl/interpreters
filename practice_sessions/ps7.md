...
A type-binding is a pair  `<T,s(T)>`  such that  `T = s(T)`. Type-Substitutions are written using set notation, for example :
...
The type inference succeeds since we have a type for T0, meaning that the expression is  **well typed**. Because there are no free variables, the inferred type of T0 is:  **Number.**
