...
נתבונן ב-continuation שנבנה לאורך החישוב:
 ...
 ![diagram](https://github.com/bguppl/interpreters/blob/gh-pages/practice_sessions/resources/ps10/diagram.png)       

 	
\$\$map מפעילה את \$f על אברי הרשימה מהסוף להתחלה (בדוגמה 5 → 4 → 3 → 2 → 1).  הפעלת f והקריאה ל-map אינן תלויות (אינן מקוננות זו בזו) ולכן ניתן לבחור מה יתרחש קודם בגרסת ה CPS. כך נקבע את הסדר בו תופעל \$f על אברי הרשימה (מההתחלה לסוף או להיפך). הגרסה הבאה מפעילה את \$f על אברי הרשימה לפי הסדר הרגיל של מההתחלה לסוף:
