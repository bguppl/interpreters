...
![equation](http://www.sciweavers.org/tex2img.php?eq=%7Bx%2Cf(x)%2Cf(f(x))%2Cf(f(f(x)))%2C...%7D&bc=White&fc=Black&im=jpg&fs=12&ff=arev&edit=)

שאלה 2 – סדרת ההפעלות העצמיות
בהינתן פונקציה *f* וערך *x*, נגדיר את סדרת ההרכבות העצמיות להיות {![equation](http://www.sciweavers.org/tex2img.php?eq=%7Bx%2Cf(x)%2Cf(f(x))%2Cf(f(f(x)))%2C...%7D&bc=White&fc=Black&im=jpg&fs=12&ff=arev&edit=)}. הסדרה
יכולה להיות סופית, או אינסופית. נרצה לייצר את הרשימה העצלה שאיבריה הם איברי הסדרה. הרשימה תיגמר כאשר
הגיעה לנקודת שבת, כלומר כאשר הערך המוחזר מהפעלת *f* הוא אותו הערך שעליו *f* הופעלה.
```typescript
