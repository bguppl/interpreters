...
4. הסימנים pipe ( `|` ), פסיק וסוגריים מרובעים מהווים חלק מ"toString" של הרשימות. למשל, `[a|A]` הוא "toString" של רשימה שהאיבר הראשון שלה הוא a והזנב הוא רשימת A. את הרשימה `[3, 5]` ניתן להציג בצורת `[3| [5| []] ]`.
...
דוגמה זו עוסקת בהשוואת זמנים המצוינים על ידי שעה ביממה ויום בשבוע. נציג ADT
(Abstract Data Type, כלומר טיפוס חדש) המייצג זמן. נבנה שלושה ערכים: שעה, יום וזמן. נשתמש בפנקטורים בתור בנאי ערך: `h(Hour)`, `d(Weekday)`, `time(Hour, Weekday)` בהתאם.
לצורך ההשוואה נספק רשימות סדורות של שעות ושל ימים. יחס הקדימות בין זמנים יקבע על פי מקומם ברשימות המסודרות של הזמנים. הפרדיקטים `weekday_list` , `hour_list` יתארו את הסדרים של ימי השבוע ושעות ביממה, בהתאמה.  
...
<table class=l><tr><td markdown=1>
```prolog
% Type: Hour.
...
hour_list([h(0),h(1),h(2),h(3),h(4),h(5),h(6),h(7),h(8),
...
weekday_list([d(sun),d(mon),d(tue),d(wed),d(thu),
d(fri),d(sat)]).
...
```
```prolog
?- merge([time(h(1),d(sun)),time(h(3),d(wed)),
                              time(h(5),d(sat))],
...
(expand-lzt node node-expander))
                     child-nodes)))) ))
```
**Getters**: `lzt->root`, `leaf-data`, `lzt->branches`, `lzt->first-branch`, `lzt->rest-branches`, `lzt->take-branches(lzt,n)`, `lzt->nth-level(lzt, n)`  
