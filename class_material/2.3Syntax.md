...
This method provides a complete operational semantics procedure for the language by applying the method of **structural induction**: by applying the computation rules over all possible expressions in the language recursively, we can map
any expression to a value.  
...
For example:
* https://www.cs.indiana.edu/scheme-repository/R4RS/r4rs_9.html#SEC76 presents the formal syntax of the R4RS Scheme programming language
* https://262.ecma-international.org/#sec-grammar-summary presents the formal syntax of JavaScript 2020
* https://docs.oracle.com/javase/specs/jls/se15/html/jls-19.html presents the formal syntax of Java 15.
* https://docs.python.org/3/reference/grammar.html presents the formal syntax of Python.
...
Consider the first few lexical rules in the Scheme specification (https://www.cs.indiana.edu/scheme-repository/R4RS/r4rs_9.html#SEC76):
...
We apply the construct of **disjoint union types** (which we introduced in [Chapter 1.4](https://www.cs.bgu.ac.il/~ppl202/wiki.files/class/notebook/1.4TypesAndOperations.html#Disjoint-Types-and-Disjoint-Union)) to define formally the abstract syntax of a language.
...
You can experiment with ASTs of existing languages in the https://astexplorer.net/ site which provides an interactive browser of ASTs for a wide range of programming languages (JavaScript, Scala, Go, SQL and many more).

### Implementing ASTs in TypeScript

Please refer to the code in https://github.com/bguppl/interpreters to review and execute the code of the implementation of ASTs and Parsers in TypeScript.
...
You can read more on the important topic of regexp in https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions
...
const parseECompound = (sexps: Sexp[]): Result<E> =>
    ...
    isString(sexps[1]) ? safe2((arg1: E, arg2: E) => parseE3(sexps[1], arg1, arg2))(parseESexp(sexps[0]), parseESexp(sexps[2])) :
