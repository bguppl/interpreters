...
In the rest of the course, we will use a functional approach to handle errors, based on the `Result<T>` monad introduced in 
__[Homework 1](https://www.cs.bgu.ac.il/~ppl202/Assignments/Assignment_1)__.  Therefore, we wrap the call to the library parser
in a function that adapts the `Error` protocol into a `Result<Sexp>`:
...
* The variable reference y in line 2 refers to the declaration in line 2;
