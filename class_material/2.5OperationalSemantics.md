...
The corresponding TypeScript implementation of this AST using the Disjoint-Union pattern and with the `parseL1` AST factory function is provided in https://github.com/bguppl/interpreters/blob/master/src/L1/L1-ast.ts which maps a string containing the concrete syntax of a program in $$L1$$ to its AST representation.

### Representing Primitive Operators

Looking at the set of values that an $$L1$$ program can compute, starting with atomic expressions, we decide to represent `Number` and `Boolean` using the corresponding value types in the meta-language (in our case, in TypeScript).
...
But in contrast, the expression `+` (where `+` can be any primitive operator) is *not* a well formed expression in TypeScript. 
...
In the implementation of $$L3$$, we will implement type checking for primitives.
...
A function is **partial** when it is defined on a restricted domain - in our case, not all variable references will be defined in a given environment, and the function is actually a **finite function** (defined on a finite set of values).
...
(see full source code in https://github.com/bguppl/interpreters/blob/master/src/L1/L1-eval.ts#L13):
...
* No variable is defined in an empty environment - in this case, we return an error (Failure).
...
This algorithm is implemented in https://github.com/bguppl/interpreters/blob/master/src/L1/L1-eval.ts#L90.
...
https://github.com/bguppl/interpreters/blob/master/src/L1/L1-eval.ts

Tests are available in https://github.com/bguppl/interpreters/blob/master/test/L1/L1-eval.test.ts
