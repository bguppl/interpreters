...
const checkEqualType = (te1: T.TExp | Error | undefined, te2: T.TExp | Error, exp: A.Exp): true | Error =>
    isError(te1) ? te1 :
    isError(te2) ? te2 :
    te1 === undefined ? Error(`Incompatible types: undefined - ${T.unparseTExp(te2)}`) :
...
export const typeofLetrec = (exp: A.LetrecExp, tenv: E.TEnv): T.TExp | Error => {
    ...
    const types = R.zipWith((bodyI, tenvI) => typeofExps(bodyI, tenvI), bodies, tenvIs)
    const constraints = R.zipWith((typeI, ti) => checkEqualType(typeI, ti, exp), types, tis)
    if (hasNoError(constraints))
